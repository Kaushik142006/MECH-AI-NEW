...
def _load_dataset() -> pd.DataFrame:
    ...
    if not os.path.exists(csv_path):
        # Fallback: try to find any CSV in data_sets
        import glob
    ...
    required = ['load_n', 'temperature_c', 'component', 'material', 'stress_pa', 'strain', 'yield_pa', 'safety']
    for col in required:
...
def _train_and_save_models():
    ...
    # Preprocessor: scale numeric, one‑hot encode component
    ...
    # Filename mapping (stress model saved as stress_ml_gen.pkl)
    filename_map = {
    ...
    for name, model in models.items():
        ...
        # Save preprocessor and pipeline together
...
def _load_ml_model(model_file: str):
    ...
    if not os.path.exists(full_path):
        # Train all models once if any is missing
...
def _predict_from_ml_models(load: float, temperature: float, component_type: str) -> Dict[str, float]:
    ...
    # Input must have exactly the same columns as training
    ...
    # Post‑process predictions to match simulation expectations
    stress_mpa = float(predictions["stress"]) / 1_000_000.0   # Pa -> MPa
